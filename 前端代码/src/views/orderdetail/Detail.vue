<template>
    ...
    >
            ...
            <el-button v-if="form.useridFrn" type="primary" text bg @click="useridDetail(form.userid)">{{ form.useridFrn.name }}</el-button>
        </el-form-item>
...
<script setup>
    ...
    let form = ref({ useridFrn:null,goodsidFrn:null,midFrn:null, });

    const render = async (id) => {
        Msg.loading("loading...")
        let {data} = await Http.get(`/orderdetail/detail`, {id});
        console.log(data)
        form.value = data;
        Msg.loading(false)
    }
