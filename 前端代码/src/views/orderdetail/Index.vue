    ...
    <x-main>
        ...
        <el-table :data="pager.records" style="width: 100%">
            ...
            <template #default="scope">
                <el-button v-if="scope.row.useridFrn" type="primary" text bg @click="useridDetail(scope.row.userid)">{{ scope.row.useridFrn.name }}</el-button>
            </template>
        </el-table-column>
            ...
            <template #default="scope">
                <span >{{scope.row.zongj}}</span>
                 ...
                 <el-table-column sortable label="时间">
            <template #default="scope">
            ...
            <el-table-column fixed="right" label="操作">
                <template #default="scope">
                    ...
                    <button class="btn btn-warn" v-if=" user.role.toLowerCase() == ''  " @click="onEdit(scope.row.id)">修改</button>
...
<script setup name="orderdetail">
    ...
    const loadThisPage = async () => {
        ...
        let {data,message} = await Http.get(`/orderdetail/list`, params);
        console.log("查询条件")
        console.log(params)
        console.log("查询结果")
        console.log(data)
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/orderdetail/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    //onExcel
    const onExcel = async () => {
        Http.download({},false,"/orderdetail/excel")
    }
    //删除
    const onDelete = async (id) => {
        const op = Msg.confirm('确定删除？')
        op.then(async () => {
            let {success, message} = await Http.post(`/orderdetail/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
    ...
    const importExcel = ()=>{
        const op = Dialog.open(ImportExcel, `导入订单详情数据`).setCancelText('').setConfirmText('')
        op.mounted(c => {
            c.render('orderdetail',loadThisPage)
        })
    }
</script>
