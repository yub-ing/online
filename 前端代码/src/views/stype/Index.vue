    ...
    <x-main>
        ...
        <el-row class="search-row">
                            ...
                            <span>类型名</span>
                                    ...
                                    <el-input clearable placeholder="请输入类型名" v-model="searchForm.name" maxlength="18"/>
                                </div>
                                                    </div>
...
<script setup name="stype">
    ...
    const ups = async (id, msg, field, value) => {
      ...
      op.then(async () => {
        ...
        let {success, message} = await Http.post(`/stype/save`, upsForm);
        if (!success) {
          Msg.error(message);
        }else{
          Msg.success(message);
    ...
    //onExcel
    const onExcel = async () => {
        Http.download({},false,"/stype/excel")
    }
    //删除
    const onDelete = async (id) => {
        const op = Msg.confirm('确定删除？')
        op.then(async () => {
            let {success, message} = await Http.post(`/stype/delete?id=` + id);
            if (!success) {
                Msg.error(message);
            } else {
                Msg.success(message);
    ...
    const importExcel = ()=>{
        const op = Dialog.open(ImportExcel, `导入商品类型数据`).setCancelText('').setConfirmText('')
        op.mounted(c => {
            c.render('stype',loadThisPage)
        })
    }
</script>
